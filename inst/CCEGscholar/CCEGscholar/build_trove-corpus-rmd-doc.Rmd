...
**Load relevant libraries:** You'll need a couple of new libraries, *httr* and *jsonlite*, as well as the usual tidyverse. *httr* allows you to interact with URL queries, and *jsonlite* will read the JSON format file which is retrieved by the query. 
...
Believe it or not, there are other collections and newspapers datasets in addition to the British Library. Trove, the National Library of Australia's content platform, is one such collection. 
...
```q=<YOUR SEARCH TERMS>``` after ```q=``` you put whatever you're searching for. Note that these queries need special characters in place of space, otherwise you'll get an error. What this means is that unless you paste together the correct special characters along with the search terms correctly (with respect to API) the API will not understand what you are asking for.

> You have to replace whitespace with ```%20```. So ```sydney city``` as a search term needs to become ```sydney%20city```. 

If you do it without the spaces in your browser, it usually fills them in automatically, but R won't do that for you. Ultimately, you get the same results as you would if you contructed a query through Trove's website, but here you can save them as objects, and get lots of results all at once. 
...
```{r}
key = 'q6ikr8sqem7hbqib' #get your own key and put it here
zone = 'newspaper' # The zone to search
query = "sydney+city" # the query we'll search for
encoding = 'json' # the format of the results
results = '10000' #how many results per query. 10 is an easy way to reduce traffic when testing things out
...
#ifelse warning run before here I reckon
our_query
```
...
First create a new variable containing just the content part of the result. Within the content section of the `API` response there are the following variables:
...
It's a raw format, which needs to be converted to characters, using the base R command ```rawToChar()```. When, it's one long character string, which is actually a JSON encoded file. Using *jsonlite* we can read this into `R`. The JSON is another complicated list. lThere will be different bits of this data that we will want to use at different times throughout a project. The bit we want to begin with is here:

```{r}
b_text <- b2$response$zone$records$article
b_records <- b2$response$zone$records

#convert to df
b_textdf <- as.data.frame(b_text)

#has to be tibble to work with select
b_heading <- b_textdf %>%
              dplyr::select(date, heading, id, title, date, page, snippet) %>%
                mutate(dateEvent = as.Date(date),
                       titleEvent = factor(heading),
                       yearEvent = lubridate::year(as.Date(date)))

table(b_heading$yearEvent)
table(b_heading$titleEvent)

#need to clean txt here...
#just for heading variable for now
#june 2021
#Anthony

# Find out what all the sentances look like:
  head(b_heading$heading)
  
#change all city and sydney headlines into a single variable indicating this
# generalheading <- c("doa", "DOA", "eaten", "dead", "killed", "owl", "Dead", "DEAD", "Died", "Killed")  
# table(b_heading$generalheading)
  
  
  
# # code to go through each word in the list and use grep to locate the rows where trap.note contains that word
  
# then set generalheading == y for those rows
...
Hurray, that looks like a normal dataframe! Next turn the dataframe into a slightly more useable format using ```as_tibble()``` and select just the columns you're interested in. Note that each cell in the title column is actually a dataframe of 1 row and two columns! Double rolleyes! We can sort that later.

```{r}
b3  <- b_heading %>%
  as_tibble(.name_repair = 'universal') %>% 
  dplyr::select(heading, title, date, page, snippet)
```
...
get_trove_articles = function(key = 'q6ikr8sqem7hbqib', zone = 'newspaper',query = 'war',encoding = 'json', totalresults = 1000, decade = '184'){
